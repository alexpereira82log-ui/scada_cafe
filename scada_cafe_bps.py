...
MES_EXIBICAO = int(input("Digite o número do Mês: "))
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
perdas_df = pd.read_excel('base_bps_perdas.xlsx')
comissao_df = pd.read_excel('base_bps_comissao.xlsx')
...
while True:
    ...
    if escolha == "1":
        ...
        # # AJUSTES FIGURA:
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        fig.suptitle(f"Dashboard Scada Café - Loja Bps - {nome_do_mes_corrente} {ANO_EXIBICAO}", fontsize=16, fontweight='bold', color='darkgrey')

        # Gerar arquivo .PNG do gráfico:
        plt.savefig("dashboard_Mes_Corrente_bps.png", dpi=300, bbox_inches="tight")
        ...
        # # AJUSTES FIGURA:
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        fig.suptitle(f"Dashboard Mês a Mês - Scada Café - Loja Bps - {ANO_EXIBICAO}", fontsize=16, fontweight='bold', color='darkgrey')

        # Gerar arquivo .PNG do gráfico:
        plt.savefig("dashboard_Mes_a_Mes_bps.png", dpi=300, bbox_inches="tight")
        ...
        # # AJUSTES FIGURA:
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        fig.suptitle(f"Dashboard - Scada Café - Loja Bps - {ANO_EXIBICAO}", fontsize=16, fontweight='bold', color='darkgrey')

        # Gerar arquivo .PNG do gráfico:
        plt.savefig("dashboard_Mes_Mural_bps.png", dpi=300, bbox_inches="tight")
        ...
        def enviar_email():
            msg = MIMEMultipart()
            msg["Subject"] = f"Relatório Scada Café - Loja Bps - {ANO_EXIBICAO}"
            ...
            <body>
            <p>Segue relatório de análises e resultados atualizado referente às vendas de Scada Café (Loja Bps).</p>
            ...
            for nome_arquivo in lista_arquivos:
                if nome_arquivo.lower().endswith("bps.png"):
